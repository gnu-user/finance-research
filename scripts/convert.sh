...
###############################################################################
#
# A helpful script for concurrently converting many files at once from
# sas7bdat format to CSV. To use with SHARCNET you can use the following:
# 
# sqsub -q threaded -r <time> -n <# cpu> --memperproc=<amount of mem> -o 
# output.log ./convert.sh <dir with sas7bdat files> <# threads (same as #cpu)>
#
# 
...
then
    ...
    echo "Usage: $0 {directory with sas7bdat files} {number of threads (optional)}" 1>&2
    echo "Example: $0 /work/user/TAQ_DATA/ 4"
    exit 1
fi
